...
parser.add_argument('--full_classifier', type=int, help='Use full classifier for evaluation or not')
...
fc = input_args.full_classifier == 1
...
if fc:
    ...
    filename = f"{exp_id}_classifier.pth"
...
for idx in range(len(latent_dims)):
    ...
    df_delta = eval_by_delta(model, classifier, decoder, dl, indices, device="cuda", idx=idx)
    ...
    suffix = "_full" if fc else ""
    results_acc.to_csv(f"results/{args.dataset}/{exp_id}_acc_detailed{suffix}.csv")